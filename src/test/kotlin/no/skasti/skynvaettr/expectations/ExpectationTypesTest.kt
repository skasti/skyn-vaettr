...
class ExpectationTypesTest {
    ...
    private val t1 = Instant.parse("2026-09-13T10:01:00Z")

    @Test
    fun `prediction carries bounded model confidence`() {
        val prediction = Prediction(signal, 21.0, 0.7, t0)

        assertEquals(signal, prediction.signal)
        assertEquals(21.0, prediction.value)
        assertEquals(0.7, prediction.confidence)
        assertEquals(t0, prediction.createdAt)

        assertFailsWith<IllegalArgumentException> { Prediction(signal, 21.0, -0.01, t0) }
        assertFailsWith<IllegalArgumentException> { Prediction(signal, 21.0, 1.01, t0) }
        assertFailsWith<IllegalArgumentException> { Prediction(signal, 21.0, Double.NaN, t0) }
    }

    @Test
    fun `expectation starts from prediction confidence`() {
        val prediction = Prediction(signal, 21.0, 0.6, t0)
    ...
    fun `expectation validates commitment values`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            Expectation(prediction, t0.minusSeconds(1), cost = 0.2, reward = 1.0)
        }
        assertFailsWith<IllegalArgumentException> {
    ...
    @Test
    fun `later matching prediction can reinforce confidence`() {
        val initial = Prediction(signal, 21.0, 0.6, t0)
        val expectation = Expectation(initial, t0, cost = 0.2, reward = 1.0)
        val reinforced = expectation.reinforcedBy(Prediction(signal, 21.0, 0.85, t1))

        assertEquals(0.85, reinforced.confidence)
        assertEquals(t0, reinforced.formedAt)
        assertEquals(t1, reinforced.prediction.createdAt)
    ...
    fun `reinforcement never lowers confidence`() {
        val initial = Prediction(signal, 21.0, 0.8, t0)
        val expectation = Expectation(initial, t0, cost = 0.2, reward = 1.0)
        val reinforced = expectation.reinforcedBy(Prediction(signal, 21.0, 0.7, t1))
    ...
    fun `reinforcement must represent the same prediction`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            expectation.reinforcedBy(Prediction(otherSignal, 21.0, 0.8, t1))
        }
        assertFailsWith<IllegalArgumentException> {
            expectation.reinforcedBy(Prediction(signal, 22.0, 0.8, t1))
        }
    }
}
